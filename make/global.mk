################################################################################
## @file:	global.mk
## @author	郭春茂 <gcm.ustc.edu>
## @brief	全局设置.
## @version	1.0
###############################################################################

include $(ROOT_MAKE_DIRECTORY)/func/dirs.mk

GLOBAL_COMPILE_FLAGS		:= $(GLOBAL_COMPILE_FLAGS) -DBOOST_USER_CONFIG=\<boost_config.h\>
GLOBAL_COMPILE_FLAGS		:= $(GLOBAL_COMPILE_FLAGS) -DFRAMEWORK_USER_CONFIG=\<framework_config.h\>

PLATFORM_LOCAL_NAME		:= /$(call revert_directory,$(ROOT_OUTPUT_DIRECTORY))

PLATFORM_STRATEGY_NAME		:= $(firstword $(subst /, ,$(PLATFORM_LOCAL_NAME)))

LOCAL_NAME			:= $(patsubst /$(PLATFORM_STRATEGY_NAME)%,%,$(PLATFORM_LOCAL_NAME))

PLATFORM_NAME			:= $(subst ., ,$(PLATFORM_STRATEGY_NAME))

STRATEGY_NAME			:= $(word 2,$(PLATFORM_NAME))

PLATFORM_NAME			:= $(firstword $(PLATFORM_NAME))

PLATFORM_DIRECTORY		:= $(ROOT_PLATFORM_DIRECTORY)/$(PLATFORM_NAME)

PLATFORM_OUTPUT_DIRECTORY	:= $(ROOT_OUTPUT_DIRECTORY)/$(PLATFORM_STRATEGY_NAME)

PLATFORM_TOOL_DIRECTORY		:= $(ROOT_TOOL_DIRECTORY)/$(PLATFORM_NAME)

include $(wildcard $(PLATFORM_DIRECTORY)/make/*.mk)

PLATFORM_TOOL_PATH		:= $(call reletive_directory,$(addprefix $(PLATFORM_TOOL_DIRECTORY),$(PLATFORM_TOOL_PATH)) $(ROOT_TOOL_DIRECTORY))
$(foreach path,$(PLATFORM_TOOL_PATH),$(eval PATH:=$(path):$(PATH)))
export PATH

GLOBAL_COMPILE_FLAGS		:= $(GLOBAL_COMPILE_FLAGS) -DPLATFORM_NAME=$(PLATFORM_NAME)
GLOBAL_COMPILE_FLAGS		:= $(GLOBAL_COMPILE_FLAGS) -DTOOL_NAME=$(PLATFORM_TOOL_NAME)
GLOBAL_COMPILE_FLAGS		:= $(GLOBAL_COMPILE_FLAGS) -DSTRATEGY_NAME=$(STRATEGY_NAME)

GLOBAL_COMPILE_FLAGS		:= $(GLOBAL_COMPILE_FLAGS) -fvisibility=hidden

GLOBAL_LINK_FLAGS		:= $(GLOBAL_LINK_FLAGS) -Wl,--exclude-libs,ALL

PLATFORM_SYS_ROOT		:= $(addprefix $(PLATFORM_TOOL_DIRECTORY),$(PLATFORM_SYS_ROOT))
PLATFORM_INCLUDE_DIRECTORYS	:= $(addprefix $(PLATFORM_TOOL_DIRECTORY),$(PLATFORM_INCLUDE_DIRECTORYS))
PLATFORM_LIBRARY_DIRECTORYS	:= $(addprefix $(PLATFORM_TOOL_DIRECTORY),$(PLATFORM_LIBRARY_DIRECTORYS))

PLATFORM_INCLUDE_DIRECTORYS	:= $(PLATFORM_INCLUDE_DIRECTORYS) $(PLATFORM_DIRECTORY)
PLATFORM_LIBRARY_DIRECTORYS	:= $(PLATFORM_LIBRARY_DIRECTORYS) $(PLATFORM_DIRECTORY)

ifneq ($(wildcard $(PLATFORM_DIRECTORY)/include),)
	PLATFORM_INCLUDE_DIRECTORYS	:=	$(PLATFORM_INCLUDE_DIRECTORYS) $(PLATFORM_DIRECTORY)/include
endif

ifneq ($(wildcard $(PLATFORM_DIRECTORY)/lib),)
	PLATFORM_LIBRARY_DIRECTORYS	:=	$(PLATFORM_LIBRARY_DIRECTORYS) $(PLATFORM_DIRECTORY)/lib
endif

ifneq ($(LOCAL_NAME),/cex)
        ifneq ($(wildcard $(PLATFORM_DIRECTORY)/cex),)
		PLATFORM_INCLUDE_DIRECTORYS	:= $(PLATFORM_INCLUDE_DIRECTORYS) $(PLATFORM_DIRECTORY)/cex
		PLATFORM_DEPENDS		:= $(PLATFORM_DEPENDS) /cex
        endif
endif

COMMON_MAKE_FILES		:= $(addprefix $(PLATFORM_OUTPUT_DIRECTORY),$(addsuffix /Common.mk,$(LOCAL_NAME) $(call root_directories,$(LOCAL_NAME))) /Common.mk)

-include $(COMMON_MAKE_FILES)

PROJECT_DIRECTORY		:= $(ROOT_DIRECTORY)$(LOCAL_NAME)

ifneq ($(LOCAL_NAME),/cex)
		MAKE_FILE		:= $(wildcard $(PROJECT_DIRECTORY)/Makefile.mk $(PROJECT_DIRECTORY)/Makefile.in)
else
		MAKE_FILE		:= $(wildcard $(ROOT_MAKE_DIRECTORY)$(LOCAL_NAME)/Makefile.mk $(ROOT_MAKE_DIRECTORY)$(LOCAL_NAME)/Makefile.in)
endif

ifneq ($(MAKE_FILE),)
		include $(firstword $(MAKE_FILE))
else
		LOCAL_TYPE		:= dirs
endif

COMMON_MAKE_FILES		:= $(addprefix $(ROOT_DIRECTORY),$(addsuffix /Common.mk,$(LOCAL_NAME) $(call root_directories,$(LOCAL_NAME))) /Common.mk)

-include $(COMMON_MAKE_FILES)

ifneq ($(STRATEGY_NAME),)
        include $(ROOT_STRATEGY_DIRECTORY)/$(STRATEGY_NAME).mk
endif

include $(wildcard $(PLATFORM_DIRECTORY)/make2/*.mk)
